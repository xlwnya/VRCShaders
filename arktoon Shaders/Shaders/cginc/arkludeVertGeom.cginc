...
void geom(triangle v2g IN[3], inout TriangleStream<VertexOutput> tristream)
{
    ...
    if (_UseOutline) {
        for (int i = 2; i >= 0; i--)
        {
            ...
            UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(IN[i]);
    ...
    if (_UseDoubleSided) {
        ...
        {
            UNITY_SETUP_INSTANCE_ID(IN[iii]);
            UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(IN[iii]);
            UNITY_TRANSFER_INSTANCE_ID(IN[iii], o);
    ...
    for (int ii = 0; ii < 3; ii++)
    {
        ...
        UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(IN[ii]);
        UNITY_TRANSFER_INSTANCE_ID(IN[ii], o);
